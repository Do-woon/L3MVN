# L3MVN → iGibson Porting Notes — v2.1 (TODO#4 완료 + 코드 검증 반영)
...
## 14. “GT semantic one-hot” 생성 절차 (L3MVN 입력 계약으로 연결)

L3MVN mapper는 `obs[:, 4:, :, :]`를 semantic one-hot(or prob) 채널로 기대한다. (v1 문서 참고)
...
실제 L3MVN 코드 (agents/sem_exp.py:390-392):
```python
...
iGibson에서 이를 만들기 위한 절차(개념):

1) `seg_id` (H,W) int32 획득 (10.1)
2) `class_id → class_name` 변환 (10.2)
3) `class_name → L3MVN_idx(1..15)` 변환 (13.x)
   - 매핑되지 않는 클래스는 0 (무시)
4) `sem_onehot` 생성:
   - shape: **(16, H, W)** ← 15가 아닌 **16채널**
   - sem_onehot[k-1, y, x] = 1 if L3MVN_idx(seg_id[y,x]) == k else 0
   - 채널 0..14: hm3d_category[0..14]에 대응 (“chair”~”stairs”)
   - 채널 15: 여분 (L3MVN에서 semantic id 16에 대응하는 항목이 있을 경우 사용,
     일반적으로 비어있어도 무방)
5) 최종 obs/state 채널에 concat:
   - **[RGB(3), Depth(1), Sem(16)] = 20채널** (L3MVN의 `Semantic_Mapping` 입력 계약 만족)
